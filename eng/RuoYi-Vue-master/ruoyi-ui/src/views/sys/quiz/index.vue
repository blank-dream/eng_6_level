  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['sys:quiz:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="quizList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="标准答案" align="center" prop="answerType" />
      <el-table-column label="解析" align="center" prop="explanation" />
      <el-table-column label="考生答案" align="center" prop="answer" />
      <el-table-column label="老师阅卷" align="center" prop="gradedBy" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="考生答案" prop="answer">
          <el-input v-model="form.answer" placeholder="请输入考生答案" />
        </el-form-item>
        <el-form-item label="老师阅卷" prop="gradedBy">
          <el-input v-model="form.gradedBy" placeholder="请输入老师阅卷" />
        </el-form-item>
...
<script>
import { listQuiz, getQuiz, delQuiz, addQuiz, updateQuiz } from "@/api/sys/quiz";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listQuiz(this.queryParams).then(response => {
        this.quizList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getQuiz(answerId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.answerId != null) {
            updateQuiz(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addQuiz(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除成绩查询编号为"' + answerIds + '"的数据项？').then(function() {
        return delQuiz(answerIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('sys/quiz/export', {
        ...this.queryParams
      }, `quiz_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
